...
#include <errno.h>
#include <net/if_dl.h>
#define SERVER_URL @"http://collector.predictiveplay.com/spoolers/"
...
#define universalKey @"predictivePlay"
...
- (void) start {
    self.defaultUserId = calculateHMAC_SHA1([self userIdentifier], universalKey);
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSHTTPURLResponse *)response {
	if ([response statusCode] != 200) {
		NSLog(@"fail status code %@", [NSHTTPURLResponse localizedStringForStatusCode:[response statusCode]]);
	} else {
